...
def _undermined_victim(board, guard_sq: str, opp: str) -> "object | None":
...
def _undermines_defender(pre_fen: str | None, uci: str | None) -> str | None:
    """The FIRST reasoner (our own XAI): derive the causal point of a move that undermines a defender —
    'By attacking the only defender of the bishop on a2, Black cannot hold everything'. Two forms of the
    same motif, both grounded on the board (attacker/defender sets), never guessed:

      REMOVE — the move CAPTURES the piece that was the sole defender of a more valuable enemy piece V,
               so V is now undefended (and falls if it is also attacked). E.g. Rxc3 takes the knight
               that alone defended the a2 bishop.
      ATTACK — the moved piece now ATTACKS a piece G that is the sole defender of an enemy piece V, and
               G is either undefended or winnable (SEE ≥ 0), so the threat is real. E.g. Rxc5 attacks
               the c3 knight, the only defender of the a2 bishop — Black cannot save both.

    Returns one sentence (the POINT), or None. Colour-absolute (the perspective anchor maps it)."""
    ...
    try:
        ...
        def word(board, sq):
            p = next((x for x in board.piece_list() if x.square == sq), None)
            return _PIECE_WORD.get((p.piece if p else "").upper(), "pawn")
        ...
        if took is not None:
            ...
            # Only 'the point' when the undermined piece is worth at LEAST the captured guard — else the
            # point is simply winning the guard, not the undermining (review #3).
            if v is not None and _PIECE_VAL.get(v.piece.upper(), 0) >= _PIECE_VAL.get(took.piece.upper(), 0):
                ...
                vword = _PIECE_WORD.get(v.piece.upper(), "pawn")
                tail = (f"{opp_cap}'s {vword} on {v.square}, which now falls" if falls
                        else f"{opp_cap}'s {vword} on {v.square}, leaving it undefended")
        ...
        for g in after.piece_list():
            if g.color != opp or g.piece.upper() == "K":   # a king-guard is a check — a different motif (review #2)
                continue
            ...
            if not winnable:
                continue
            gword, vword = _PIECE_WORD.get(g.piece.upper(), "pawn"), _PIECE_WORD.get(v.piece.upper(), "pawn")
            # A THREAT, not a proven win — after this it is the OPPONENT's move and they may reinforce the
            # guard or add a defender to V (review #1). Phrase the pressure honestly, don't assert the win.
            return (f"The point of this move: it attacks the {gword} on {g.square}, the only defender of "
                    f"{opp_cap}'s {vword} on {v.square} — threatening to win it and leave that {vword} loose.")
        return None
    except Exception:
        return None
