...
class VerdictPrompt:
    ...
    _WRONG = (
        ...
        "(d) THE DEEPER LINE — if the refutation line goes further (you grab material back, the "
        "opponent answers with a check that flips it) OR the facts list a key defensive resource (a "
        "saving check like Rh1+, a mutually-defending pair), surface it: show that even the natural "
        "recovery fails, so the player sees the whole trap, not just the first capture.\n"
