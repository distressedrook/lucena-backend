...
class VerdictPrompt:
    ...
    _RIGHT = (
        ...
        "LIST: one \"- \" bullet per point below, one clause each, skipping any the facts don't support. "
        "If nothing beyond the lead applies, stop after the sentence (no list). Write the SUBSTANCE only "
        "— NEVER print a category name or label prefix ('The point:', 'The threat:'); the labels below "
        "are guidance for YOU, not text to emit. Use EVERY specific insight the facts give you; do not "
        "flatten a rich point to a bare 'nice, that wins the piece'. Cover, in order:\n"
        "(a) the threat the move creates — if the facts say the move CREATES A THREAT ('creates this "
        "threat: White threatens mate: Ra4#'), name it as the concrete reward. Never state a threat the "
        "facts do not name.\n"
        "(b) the point — the instructive heart: if the facts name the position's linchpin or the "
        ...
        "move. If the facts name no such point, skip it.\n"
    ...
    _WRONG = (
        ...
        "THEN — a blank line, then a SHORT BULLETED LIST: one \"- \" bullet per point below, one clause "
        "each, in order, skipping any the facts don't support. Write the SUBSTANCE only — NEVER print a "
        "category name or label prefix ('The refutation:', 'Why it works:', 'The fix:'); the labels "
        "below are guidance for YOU, not text to emit. Use EVERY specific insight the facts give you; do "
        "not reduce a rich explanation to a bare 'you lose the piece'. Cover, in order:\n"
        ...
        "(c) why it works — the facts give ONE exact mechanism (a defender was walked off; the piece "
        "moved onto a square the opponent still GUARDS; a piece was already hanging; or the move only "
        "DRAWS by STALEMATE — the opponent left with no legal move, so the win had to keep them a spare "
        "tempo). Use THAT specific reason — not a generic 'you left it undefended'; the wrong mechanism "
        "is a grounding error. If the facts say it stalemates, lead with it.\n"
