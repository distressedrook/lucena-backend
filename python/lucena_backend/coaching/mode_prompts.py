...
class VerdictPrompt:
    ...
    _RIGHT = (
        ...
        "mating net, the winning resource, the positional gain). If the facts say the move CREATES A "
        "THREAT ('creates this threat: White threatens mate: Ra4#'), name it — that is the concrete "
        "reward of the move. Never state a threat the facts do not name.\n"
    ...
    _WRONG = (
        ...
        "to do, do not praise the move, and do not describe where the piece was heading or its supposed "
        "purpose ('moving toward the center', 'bringing the king to safety', 'an ambitious try') — none "
        "of that is in the facts, and inventing it is a grounding error. Go straight to the refutation.\n"
        ...
        "it FIRST in one clause — the move is not senseless, it really does threaten that — then show "
        "how the refutation defuses it. Never state the threat unless the facts name it.\n"
        ...
        "hanging; or the move only DRAWS by STALEMATE — the opponent is left with no legal move, so "
        "the win had to keep them a spare tempo). Use THAT specific reason — do NOT fall back on a "
        "generic 'you left it undefended' when the facts say otherwise; the wrong mechanism is a "
        "grounding error. If the facts say the move stalemates, that is the whole point: lead with it.\n"
        "(d) THE DEEPER LINE — ONLY if the refutation line the facts give literally continues past the "
        "first move (you grab material back, the opponent answers with a check), walk those EXACT moves "
        "to show the recovery also fails. Never invent a continuation, a fork, or a 'mate in N' that is "
        "not written in the line — if the line is one or two moves, stop there; a short line means a "
        "short answer. You MAY also cite a key defensive resource ONLY if the facts name it.\n"
        "(e) THE FIX + RESULT — if the facts point at what to do first ('that bishop is what you must "
